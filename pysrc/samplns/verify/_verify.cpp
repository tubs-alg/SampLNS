 ...
 * @brief This file is just a quick native checking if two samples have the same coverage. The Python implementation is too slow for larger instances.
 * @version 0.1
 * @date 2023-07-18
 * 
...
bool have_equal_coverage(Sample sample_a, Sample sample_b, std::vector<std::string> concrete_features) {
        ...
        for(int v2=v1+1; v2<n; v2++) {
            for(bool v1_val : {true, false}) {
                for(bool v2_val : {true, false}) {
                    ...
                    if(a != b) {
                        std::cout << "Missing tuple: " << concrete_features[v1] << "=" << v1_val << ", " << concrete_features[v2] << "=" << v2_val << std::endl;
                        return false;
                    }
                }
            }
...
PYBIND11_MODULE(_verify, m) {
  namespace py = pybind11;
    ...
    m.def("have_equal_coverage", &have_equal_coverage, "Check if two samples have the same coverage.");
}