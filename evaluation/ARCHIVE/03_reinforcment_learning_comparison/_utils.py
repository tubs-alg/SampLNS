...
def parse_sample(instance: Instance, instance_name: str, method: str):
    with open(
        f"00_data/feature_models/{instance_name}/{instance_name}_sample_{method}.csv"
    ) as f:
        lines = f.readlines()
        sample = [{} for _ in lines[0].split(",")[1:]]
...
def get_instance(instance_name: str):
    return parse_dimacs(
        f"00_data/feature_models/{instance_name}/{instance_name}.dimacs"
    )
