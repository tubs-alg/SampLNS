...
import subprocess, shutil, os, tempfile, gzip
...
class GurobiConan(ConanFile):
    ...
    def generate(self):
        ...
        if c_imp_location:
            tc.variables["GUROBI_C_SHARED_IMP_LOCATION"] = td + '/' + c_imp_location.replace('\\', '/')
        if self.settings.get_safe('os') == "Windows":
    ...
    def _get_config_entry(self):
        ...
        if target_version not in data["versions"]:
            raise ConanInvalidConfiguration(f"Build for unknown/unsupported version {target_version} requested")
        data = data["versions"][target_version]
        if target_os not in data:
            raise ConanInvalidConfiguration(f"Target OS {target_os} not supported for version {target_version}")
        data = data[target_os]
        if target_arch not in data:
            raise ConanInvalidConfiguration(f"Unknown or unsupported architecture {target_arch} for OS {target_os} and version {target_version}")
    ...
    def _extract_msi(self, config, archive):
        ...
        with tempfile.TemporaryDirectory(suffix="_gurobi_build") as tmp:
            ...
            if len(results) != 2:
                raise RuntimeError("There should be exactly two binary_paths: dll and lib (in that order)!")
            for entry in config["cpp_source_dirs"]:
                self._copy_directory(entry["path"], tmp, entry["extract_to"])
