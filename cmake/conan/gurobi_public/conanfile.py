...
class GurobiConan(ConanFile):
    ...
    def _get_config_entry(self):
        ...
        if target_version not in cdata:
            msg = f"Unknown or unsupported version {target_version}"
            raise ConanInvalidConfiguration(msg)
        cdata = cdata[target_version]
        if target_os not in cdata:
            msg = f"Unknown or unsupported operating system {target_os} for version {target_version}"
            raise ConanInvalidConfiguration(msg)
        cdata = cdata[target_os]
        if target_arch not in cdata:
            msg = f"Unknown or unsupported architecture for OS {target_os} and version {target_version}"
            raise ConanInvalidConfiguration(msg)
